...
struct string_option {
  ...
  string_option(const std::string_view& name, const std::string& def)
...
struct spin_option {
  ...
  spin_option(const std::string& name, const spin_range& range)
      : name_{name}, range_{range} {}
  spin_option(const std::string& name, const int def)
      : name_{name}, default_{def} {}
  spin_option(const std::string& name, const int def, const spin_range& range)
      : name_{name}, default_{def}, range_{range} {}
};
...
std::ostream& operator<<(std::ostream& ostr, const spin_option& opt) {
  ...
  if (opt.range_.has_value()) {
    ostr << " min " << opt.range_.value().min << " max "
         << opt.range_.value().max;
  }
  return ostr;
}
...
inline constexpr bool is_option_v =
    std::is_same_v<T, spin_option> || std::is_same_v<T, string_option> ||
...
struct option_callback {
  ...
  void maybe_call(const std::string& cmd) {
    ...
    if (read.has_value()) {
      callback_(read.value());
    }
  }

  template <typename F>
  option_callback(const T& option, F&& f)
      : option_{option}, callback_{f} {}
};
