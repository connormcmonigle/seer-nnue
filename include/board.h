...
constexpr size_t us_offset(piece_type pt) {
  switch (pt) {
    case piece_type::pawn:
      return us_pawn_offset;
    case piece_type::knight:
      return us_knight_offset;
    case piece_type::bishop:
      return us_bishop_offset;
    case piece_type::rook:
      return us_rook_offset;
    case piece_type::queen:
      return us_queen_offset;
    case piece_type::king:
      return us_king_offset;
    default:
      return us_pawn_offset;
  }
}
...
constexpr size_t them_offset(piece_type pt) {
  switch (pt) {
    case piece_type::pawn:
      return them_pawn_offset;
    case piece_type::knight:
      return them_knight_offset;
    case piece_type::bishop:
      return them_bishop_offset;
    case piece_type::rook:
      return them_rook_offset;
    case piece_type::queen:
      return them_queen_offset;
    case piece_type::king:
      return them_king_offset;
    default:
      return them_pawn_offset;
  }
}
...
constexpr T material_value(const piece_type& pt) {
  constexpr std::array<T, 6> values = {100, 300, 300,
                                       450, 900, std::numeric_limits<T>::max()};
...
struct board {
  ...
  std::tuple<square_set, square_set> checkers(const square_set& occ) const {
    const auto b_check_mask =
        bishop_attack_tbl.look_up(man_.us<c>().king().item(), occ);
    const auto r_check_mask =
        rook_attack_tbl.look_up(man_.us<c>().king().item(), occ);
    const auto n_check_mask =
        knight_attack_tbl.look_up(man_.us<c>().king().item());
    const auto p_check_mask =
        pawn_attack_tbl<c>.look_up(man_.us<c>().king().item());
    const auto q_check_mask = b_check_mask | r_check_mask;

    const auto b_checkers =
        (b_check_mask & (man_.them<c>().bishop() | man_.them<c>().queen()));
    const auto r_checkers =
        (r_check_mask & (man_.them<c>().rook() | man_.them<c>().queen()));
    ...
    const auto checkers_ = (b_check_mask & man_.them<c>().bishop() & occ) |
  ...
  square_set king_danger() const {
    const square_set occ =
        (man_.white.all() | man_.black.all()) & ~man_.us<c>().king();
    square_set k_danger{};
  ...
  square_set pinned() const {
    ...
    for (const auto sq :
         (k_x_hori & (man_.them<c>().queen() | man_.them<c>().rook()))) {
      pinned_set |=
          r_check_mask & rook_attack_tbl.look_up(sq, occ) & man_.us<c>().all();
    }
    for (const auto sq :
         (k_x_diag & (man_.them<c>().queen() | man_.them<c>().bishop()))) {
      pinned_set |= b_check_mask & bishop_attack_tbl.look_up(sq, occ) &
                    man_.us<c>().all();
    }
    return pinned_set;
  }
  ...
  move_list& append_en_passant(move_list& mv_ls) const {
    if (lat_.them<c>().ep_mask().any()) {
      ...
      const square_set enemy_pawn_mask =
          pawn_push_tbl<them_<c>::value>.look_up(ep_square, square_set{});
      const square_set from_mask =
          pawn_attack_tbl<them_<c>::value>.look_up(ep_square) &
          man_.us<c>().pawn();
      for (const auto from : from_mask) {
        const square_set occ_ =
            (occ & ~square_set{from.bit_board()} & ~enemy_pawn_mask) |
            lat_.them<c>().ep_mask();
        if (!std::get<0>(checkers<c>(occ_)).any()) {
          mv_ls.add_(from, ep_square, piece_type::pawn, false, piece_type::pawn,
                     true, enemy_pawn_mask.item());
        }
      }
    }
    return mv_ls;
  ...
  move_list generate_moves_() const {
    ...
    if (num_checkers == 0) {
      ...
      for (const auto from : (man_.us<c>().pawn() & ~pinned_)) {
        ...
        const auto to_loud =
            pawn_attack_tbl<c>.look_up(from) & man_.them<c>().all();
        if
          constexpr(gen_quiet) {
            for (const auto to : (to_quiet & ~last_rank)) {
              result.add_(from, to, piece_type::pawn);
            }
          }
        for (const auto to : (to_loud & ~last_rank)) {
          result.add_(from, to, piece_type::pawn, true, man_.them<c>().occ(to));
        }
        for (const auto to : (to_quiet & last_rank)) {
          result.add_promotion_<gen_quiet>(from, to, piece_type::pawn);
        }
        for (const auto to : (to_loud & last_rank)) {
          result.add_promotion_<gen_quiet>(from, to, piece_type::pawn, true,
                                           man_.them<c>().occ(to));
        }
      }
      for (const auto from : (man_.us<c>().knight() & ~pinned_)) {
        ...
        if
          constexpr(gen_quiet) {
            for (const auto to : (to_mask & ~occ)) {
              result.add_(from, to, piece_type::knight);
            }
          }
        for (const auto to : (to_mask & man_.them<c>().all())) {
          result.add_(from, to, piece_type::knight, true,
                      man_.them<c>().occ(to));
        }
      }
      for (const auto from : (man_.us<c>().rook() & ~pinned_)) {
        ...
        if
          constexpr(gen_quiet) {
            for (const auto to : (to_mask & ~occ)) {
              result.add_(from, to, piece_type::rook);
            }
          }
        for (const auto to : (to_mask & man_.them<c>().all())) {
      ...
      for (const auto from : (man_.us<c>().bishop() & ~pinned_)) {
        ...
        if
          constexpr(gen_quiet) {
            for (const auto to : (to_mask & ~occ)) {
              result.add_(from, to, piece_type::bishop);
            }
          }
        for (const auto to : (to_mask & man_.them<c>().all())) {
          result.add_(from, to, piece_type::bishop, true,
                      man_.them<c>().occ(to));
        }
      }
      for (const auto from : (man_.us<c>().queen() & ~pinned_)) {
        const auto to_mask = bishop_attack_tbl.look_up(from, occ) |
                             rook_attack_tbl.look_up(from, occ);
        if
          constexpr(gen_quiet) {
            for (const auto to : (to_mask & ~occ)) {
              result.add_(from, to, piece_type::queen);
            }
          }
        for (const auto to : (to_mask & man_.them<c>().all())) {
          result.add_(from, to, piece_type::queen, true,
                      man_.them<c>().occ(to));
        }
      }
      if (lat_.us<c>().oo() &&
          !(castle_info<c>.oo_mask & (king_danger_ | occ)).any()) {
        result.add_(castle_info<c>.start_king, castle_info<c>.oo_rook,
                    piece_type::king, true, piece_type::rook);
      }
      if (lat_.us<c>().ooo() &&
          !(castle_info<c>.ooo_danger_mask & king_danger_).any() &&
          !(castle_info<c>.ooo_occ_mask & occ).any()) {
        result.add_(castle_info<c>.start_king, castle_info<c>.ooo_rook,
                    piece_type::king, true, piece_type::rook);
      }
      if (pinned_.any()) {
        for (const auto from : (man_.us<c>().pawn() & pinned_ & k_x_diag)) {
          ...
          for (const auto to : (to_mask & ~last_rank & man_.them<c>().all())) {
            result.add_(from, to, piece_type::pawn, true,
                        man_.them<c>().occ(to));
          }
          for (const auto to : (to_mask & last_rank & man_.them<c>().all())) {
            result.add_promotion_<gen_quiet>(from, to, piece_type::pawn, true,
        ...
        for (const auto from : (man_.us<c>().rook() & pinned_ & k_x_hori)) {
          ...
          for (const auto to : (to_mask & man_.them<c>().all())) {
            result.add_(from, to, piece_type::rook, true,
                        man_.them<c>().occ(to));
          }
        }
      ...
      for (const auto from : (man_.us<c>().knight() & ~pinned_)) {
        ...
        for (const auto to : to_quiet) {
          result.add_(from, to, piece_type::knight);
        }
        for (const auto to : to_loud) {
          result.add_(from, to, piece_type::knight, true, checker_type);
        }
      }
      for (const auto from : (man_.us<c>().rook() & ~pinned_)) {
        const auto to_mask = rook_attack_tbl.look_up(from, occ);
        ...
        for (const auto to : to_loud) {
          result.add_(from, to, piece_type::rook, true, checker_type);
        }
      }
      for (const auto from : (man_.us<c>().bishop() & ~pinned_)) {
        const auto to_mask = bishop_attack_tbl.look_up(from, occ);
        ...
        for (const auto to : to_loud) {
          result.add_(from, to, piece_type::bishop, true, checker_type);
        }
      }
      for (const auto from : (man_.us<c>().queen() & ~pinned_)) {
        const auto to_mask = bishop_attack_tbl.look_up(from, occ) |
        ...
        for (const auto to : to_loud) {
          result.add_(from, to, piece_type::queen, true, checker_type);
        }
      }
    }
    const square_set to_mask =
        ~king_danger_ & king_attack_tbl.look_up(man_.us<c>().king().item());
    ...
    for (const square to : (to_mask & man_.them<c>().all())) {
      result.add_(man_.us<c>().king().item(), to, piece_type::king, true,
                  man_.them<c>().occ(to));
    }
  ...
  move_list generate_moves() const {
    return turn() ? generate_moves_<color::white, true>()
                  : generate_moves_<color::black, true>();
  }

  move_list generate_loud_moves() const {
    return turn() ? generate_moves_<color::white, false>()
                  : generate_moves_<color::black, false>();
  }
  ...
  T see_(const move& mv) const {
    ...
    for (;;) {
      {
        const auto[p, sq] =
            least_valuable_attacker<them_<c>::value>(tgt_sq, used_mask);
        if (sq == tgt_sq) {
          break;
        }
    ...
    T delta_sum{};
    for (auto iter = material_deltas.rend() - last_idx;
         iter != material_deltas.rend(); ++iter) {
    ...
    const T base = [&] {
      ...
      if (mv.is_promotion()) {
        val +=
            material_value<T>(mv.promotion()) - material_value<T>(mv.piece());
      }
      if (mv.is_capture() && !mv.is_castle_ooo<c>() && !mv.is_castle_oo<c>()) {
        val += material_value<T>(mv.captured());
      }
      return val;
    }();
  ...
  bool has_non_pawn_material() const {
    return man_.us(turn()).knight().any() || man_.us(turn()).bishop().any() ||
           man_.us(turn()).rook().any() || man_.us(turn()).queen().any();
  }

  template <color c>
  bool is_passed_push_(const move& mv) const {
    return ((mv.piece() == piece_type::pawn && !mv.is_capture()) &&
            !(man_.them<c>().pawn() & passer_tbl<c>.mask(mv.to())).any());
  }

  bool is_passed_push(const move& mv) const {
    return turn() ? is_passed_push_<color::white>(mv)
                  : is_passed_push_<color::black>(mv);
  }
  ...
  bool is_trivially_drawn() const {
    return (num_pieces() == 2) || ((num_pieces() == 3) &&
                                   (man_.white.knight() | man_.white.bishop() |
                                    man_.black.knight() | man_.black.bishop())
                                       .any());
  }
  ...
  board forward_(const move& mv) const {
    ...
    if (mv.is_null()) {
      ...
      cpy.man_.us<c>().remove_piece(piece_type::king,
                                    castle_info<c>.start_king);
      cpy.man_.us<c>().remove_piece(piece_type::rook, castle_info<c>.ooo_rook);
      cpy.man_.us<c>().add_piece(piece_type::king,
                                 castle_info<c>.after_ooo_king);
      cpy.man_.us<c>().add_piece(piece_type::rook,
                                 castle_info<c>.after_ooo_rook);
      ...
      cpy.man_.us<c>().add_piece(piece_type::king,
                                 castle_info<c>.after_oo_king);
      cpy.man_.us<c>().add_piece(piece_type::rook,
                                 castle_info<c>.after_oo_rook);
    } else {
      ...
      if (mv.is_capture()) {
        ...
        cpy.lat_.us<c>().set_ep_mask(
            pawn_push_tbl<them_<c>::value>.look_up(mv.to(), square_set{}).item());
      }
  ...
  board mirrored() const {
    ...
    over_types([&mirror, this](const piece_type& pt) {
      for (const auto sq : man_.white.get_plane(pt).mirrored()) {
        mirror.man_.black.add_piece(pt, sq);
      }
      for (const auto sq : man_.black.get_plane(pt).mirrored()) {
        mirror.man_.white.add_piece(pt, sq);
      }
    });
    // latent
    ...
    if (lat_.black.ep_mask().any()) {
      mirror.lat_.white.set_ep_mask(lat_.black.ep_mask().mirrored().item());
    }
    if (lat_.white.ep_mask().any()) {
      mirror.lat_.black.set_ep_mask(lat_.white.ep_mask().mirrored().item());
    }
  ...
  void show_feature_indices(U& updatable) const {
    ...
    // us
    for (const auto sq : man_.us<c>().pawn()) {
      updatable.template us<c>().insert(major * king_idx + us_pawn_offset +
                                        sq.index());
    }
    for (const auto sq : man_.us<c>().knight()) {
      updatable.template us<c>().insert(major * king_idx + us_knight_offset +
                                        sq.index());
    }
    for (const auto sq : man_.us<c>().bishop()) {
      updatable.template us<c>().insert(major * king_idx + us_bishop_offset +
                                        sq.index());
    }
    for (const auto sq : man_.us<c>().rook()) {
      updatable.template us<c>().insert(major * king_idx + us_rook_offset +
                                        sq.index());
    }
    for (const auto sq : man_.us<c>().queen()) {
      updatable.template us<c>().insert(major * king_idx + us_queen_offset +
                                        sq.index());
    }
    for (const auto sq : man_.us<c>().king()) {
      updatable.template us<c>().insert(major * king_idx + us_king_offset +
                                        sq.index());
    }
    // them
    for (const auto sq : man_.them<c>().pawn()) {
      updatable.template us<c>().insert(major * king_idx + them_pawn_offset +
                                        sq.index());
    }
    for (const auto sq : man_.them<c>().knight()) {
      updatable.template us<c>().insert(major * king_idx + them_knight_offset +
                                        sq.index());
    }
    for (const auto sq : man_.them<c>().bishop()) {
      updatable.template us<c>().insert(major * king_idx + them_bishop_offset +
                                        sq.index());
    }
    for (const auto sq : man_.them<c>().rook()) {
      updatable.template us<c>().insert(major * king_idx + them_rook_offset +
                                        sq.index());
    }
    for (const auto sq : man_.them<c>().queen()) {
      updatable.template us<c>().insert(major * king_idx + them_queen_offset +
                                        sq.index());
    }
    for (const auto sq : man_.them<c>().king()) {
      updatable.template us<c>().insert(major * king_idx + them_king_offset +
                                        sq.index());
    }
  }
  ...
  void show_delta(const move& mv, U& updatable) const {
    ...
    if (mv.piece() == piece_type::king) {
      ...
      updatable.template us<c>().erase(
          major * our_king_idx + mv.from().index() + us_offset(mv.piece()));
      updatable.template them<c>().erase(
          major * their_king_idx + mv.from().index() + them_offset(mv.piece()));
      if (mv.is_promotion<c>()) {
        updatable.template us<c>().insert(
            major * our_king_idx + mv.to().index() + us_offset(mv.promotion()));
        updatable.template them<c>().insert(major * their_king_idx +
                                            mv.to().index() +
                                            them_offset(mv.promotion()));
      } else {
        updatable.template us<c>().insert(
            major * our_king_idx + mv.to().index() + us_offset(mv.piece()));
        updatable.template them<c>().insert(
            major * their_king_idx + mv.to().index() + them_offset(mv.piece()));
      }
      if (mv.is_enpassant()) {
        updatable.template them<c>().erase(major * their_king_idx +
                                           mv.enpassant_sq().index() +
                                           us_pawn_offset);
        updatable.template us<c>().erase(major * our_king_idx +
                                         mv.enpassant_sq().index() +
                                         them_pawn_offset);
      }
      if (mv.is_capture()) {
        updatable.template them<c>().erase(major * their_king_idx +
                                           mv.to().index() +
                                           us_offset(mv.captured()));
        updatable.template us<c>().erase(major * our_king_idx +
                                         mv.to().index() +
                                         them_offset(mv.captured()));
      }
    }
  }
  ...
  std::tuple<position_history, board> after_uci_moves(
    ...
    while (move_stream >> move_name) {
      ...
      const auto it = std::find_if(
          list.begin(), list.end(),
          [=](const move& mv) { return mv.name(bd.turn()) == move_name; });
      assert((it != list.cend()));
  ...
  std::string fen() const {
    ...
    if (lat_.white.oo()) {
      castle_rights.push_back('K');
    }
    if (lat_.white.ooo()) {
      castle_rights.push_back('Q');
    }
    if (lat_.black.oo()) {
      castle_rights.push_back('k');
    }
    ...
    fen.push_back(' ');
    fen.append(lat_.them(turn()).ep_mask().any()
                   ? lat_.them(turn()).ep_mask().item().name()
                   : "-");
    fen.push_back(' ');
  ...
  static board parse_fen(const std::string& fen) {
    ...
    fen_pos.lat_.move_count = static_cast<size_t>(
        2 * (std::stoi(ply) - 1) + static_cast<int>(side != "w"));
    return fen_pos;
  }
};
