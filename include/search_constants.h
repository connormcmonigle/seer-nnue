...
struct fixed_constants {
  ...
  constexpr counter_type history_quiet_prune_threshold(const depth_type& depth) const { return -1024 * static_cast<counter_type>(depth * depth); }
  constexpr counter_type history_noisy_prune_threshold(const depth_type& depth) const { return -2048 * static_cast<counter_type>(depth * depth); }
