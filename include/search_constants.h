...
struct fixed_constants {
  ...
  constexpr depth_type early_depth() const { return 9; }
  ...
  constexpr score_type early_margin(const depth_type& depth, const depth_type& lmr_depth) const {
    assert(depth > 0);
    constexpr score_type m = 328;
    constexpr score_type b = 164;
    return m * static_cast<score_type>(depth - lmr_depth) + b;
  }
