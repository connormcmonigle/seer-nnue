...
struct tuning_constants{
  ...
  auto options(){
    ...
    auto option_reduce_depth = option_callback(spin_option("reduce_depth_", reduce_depth_, spin_range{2, 8}), [this](const int d){
      reduce_depth_ = d;
    });

    auto option_aspiration_depth = option_callback(spin_option("aspiration_depth_", aspiration_depth_, spin_range{3, 8}), [this](const int d){
      aspiration_depth_ = d;
    });

    auto option_nmp_depth = option_callback(spin_option("nmp_depth_", nmp_depth_, spin_range{2, 6}), [this](const int d){
      nmp_depth_ = d;
    });

    auto option_history_prune_depth = option_callback(spin_option("history_prune_depth_", history_prune_depth_, spin_range{4, 24}), [this](const int d){
      history_prune_depth_ = d;
    });

    auto option_snmp_depth = option_callback(spin_option("snmp_depth_", snmp_depth_, spin_range{2, 16}), [this](const int d){
      snmp_depth_ = d;
    });

    auto option_futility_prune_depth = option_callback(spin_option("futility_prune_depth_", futility_prune_depth_, spin_range{2, 16}), [this](const int d){
      futility_prune_depth_ = d;
    });

    auto option_history_extension_depth = option_callback(spin_option("history_extension_depth_", history_extension_depth_, spin_range{6, 16}), [this](const int d){
      history_extension_depth_ = d;
    });

    auto option_R_bias = option_callback(spin_option("R_bias_", R_bias_, spin_range{1, 12}), [this](const int d){
      R_bias_ = d;
    });

    auto option_R_div = option_callback(spin_option("R_div_", R_div_, spin_range{1, 12}), [this](const int d){
      R_div_ = d;
    });

    auto option_history_prune_threshold_mul = option_callback(spin_option("history_prune_threshold_mul_", history_prune_threshold_mul_, spin_range{-512, 0}), [this](const int d){
      history_prune_threshold_mul_ = d;
    });

    auto option_history_extension_threshold = option_callback(spin_option("history_extension_threshold_", history_extension_threshold_, spin_range{12288, 49152}), [this](const int d){
      history_extension_threshold_ = d;
    });

    auto option_futility_margin_mul = option_callback(spin_option("futility_margin_mul_", futility_margin_mul_, spin_range{512, 4096}), [this](const int d){
    ...
    auto option_snmp_margin_bias = option_callback(spin_option("snmp_margin_bias_", snmp_margin_bias_, spin_range{0, 384}), [this](const int d){
      snmp_margin_bias_ = d;
    });

    auto option_history_reduction_div = option_callback(spin_option("history_reduction_div_", history_reduction_div_, spin_range{2048, 8192}), [this](const int d){
      history_reduction_div_ = d;
    });
    ...
    return uci_options(
      option_reduce_depth,
      option_aspiration_depth,
      option_nmp_depth,
      option_history_prune_depth,
      option_snmp_depth,
      option_futility_prune_depth,
      option_history_extension_depth,
      option_R_bias,
      option_R_div,
      option_history_prune_threshold_mul,
      option_history_extension_threshold,
      option_futility_margin_mul,
      option_snmp_margin_mul,
      option_snmp_margin_bias,
      option_history_reduction_div,
      option_lmr_tbl_bias,
      option_lmr_tbl_div
    );
  }
