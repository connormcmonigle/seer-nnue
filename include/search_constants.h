...
struct fixed_constants {
  ...
  constexpr depth_type nmp_reduction(const depth_type& depth, const score_type& eval, const score_type& beta) const {
    return 3 + static_cast<depth_type>(std::min(3, (eval - beta) / 768)) + depth / 5;
  }
