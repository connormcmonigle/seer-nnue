...
struct transposition_table_entry {
  ...
  transposition_table_entry(const zobrist::hash_type& key,
                            const bound_type& type,
                            const search::score_type& score,
                            const chess::move& mv,
                            const search::depth_type& depth)
  ...
  transposition_table_entry(const zobrist::hash_type& k,
                            const zobrist::hash_type& v,
                            const search::depth_type& depth)
...
struct transposition_table {
  ...
  static constexpr size_t one_mb =
      (static_cast<size_t>(1) << static_cast<size_t>(20)) /
      sizeof(transposition_table_entry);
  std::vector<transposition_table_entry> data;
  std::atomic<transposition_table_entry::generation_type> current_gen{0};

  std::vector<transposition_table_entry>::const_iterator begin() const {
    return data.cbegin();
  }
  std::vector<transposition_table_entry>::const_iterator end() const {
    return data.cend();
  }
  ...
  void clear() {
    std::transform(data.begin(), data.end(), data.begin(),
                   [](auto) { return transposition_table_entry{}; });
  }

  void update_gen() {
    constexpr auto limit =
        std::numeric_limits<transposition_table_entry::generation_type>::max();
  ...
  __attribute__((no_sanitize("thread"))) size_t find_idx(
      const zobrist::hash_type& hash, const size_t& base_idx) const {
  ...
  __attribute__((no_sanitize("thread"))) size_t replacement_idx(
      const zobrist::hash_type& hash, const size_t& base_idx) {
    auto heuristic = [this](const size_t& idx) {
      constexpr int m0 = 1;
      constexpr int m1 = 512;
      return m0 * data[idx].depth() -
             m1 * static_cast<int>(current_gen.load() != data[idx].gen);
    };
  ...
  __attribute__((no_sanitize("thread"))) transposition_table& insert(
      const transposition_table_entry& entry) {
  ...
  std::optional<transposition_table_entry>
  find(const zobrist::hash_type& key) const {
    ...
    return (key == (result.key() ^ result.value())) ? std::optional(result)
                                                    : std::nullopt;
  }

  transposition_table(size_t size)
      : data(size * one_mb - ((size * one_mb) % bucket_size)) {}
};
}
