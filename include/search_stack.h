...
struct stack {
  ...
  size_t occurrences(const size_t& height,
                     const zobrist::hash_type& hash) const {
  ...
  std::string pv_string() const {
    ...
    for (const auto& pv_mv : future_[0].pv_) {
      if (!bd.generate_moves().has(pv_mv)) {
        break;
      }
  ...
  stack(const chess::position_history& past, const chess::board& present)
      : past_{past}, present_{present} {}
};
...
struct stack_view {
  ...
  bool improving() const {
    return (height_ > 1) &&
           (view_->at_(height_ - 2)).eval_ < (view_->at_(height_)).eval_;
  }
  ...
  const stack_view& prepend_to_pv(const chess::move& pv_mv) const {
    ...
    std::copy(child_pv.cbegin(),
              child_pv.cbegin() + std::distance(output_iter, our_pv.end()),
              output_iter);
    return *this;
  }
