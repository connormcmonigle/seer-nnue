...
struct time_manager {
  ...
  std::tuple<named_numeric_param<go::wtime>, named_numeric_param<go::btime>,
             named_numeric_param<go::winc>, named_numeric_param<go::binc>,
             named_numeric_param<go::movetime>,
             named_numeric_param<go::movestogo>, named_numeric_param<go::depth>,
             named_condition<go::infinite> > params_{};
  ...
  auto get() const {
    constexpr std::string_view name_idx =
        std::tuple_element<idx, decltype(params_)>::type::name_;
    if
      constexpr(name_idx == N::name) { return std::get<idx>(params_).data(); }
    else {
      return get<N, idx + 1>();
    }
  }
  ...
  bool should_stop(const search_info& info) {
    std::lock_guard<std::mutex> access_lk(access_mutex_);
    if (info.depth >= search::max_depth_) {
      return true;
    }
    ...
    if (elapsed() >= min_budget && info.is_stable) {
      return true;
    }
    return false;
  }
