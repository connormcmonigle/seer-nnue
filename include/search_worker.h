...
struct search_worker {
  ...
  auto pv_search(
    ...
    const bool try_probcut = !is_pv && depth >= 5 &&
                             !(maybe.has_value() && maybe->depth() >= probcut_depth &&
                               (maybe->bound() == bound_type::upper || maybe->bound() == bound_type::exact) && maybe->score() < probcut_beta);
