...
struct uci {
  ...
  void weights_info_string() {
    ...
    os << "info string loaded weights with signature 0x" << std::hex
       << weights_.signature() << std::dec << std::endl;
  }

  auto options() {
    auto weight_path = option_callback(
        string_option("Weights", std::string(default_weight_path)),
        [this](const std::string& path) {
          if (path == std::string(default_weight_path)) {
            nnue::embedded_weight_streamer<weight_type> embedded(
                embed::weights_file_data);
    ...
    auto hash_size = option_callback(
        spin_option("Hash", default_hash_size, spin_range{1, 65536}),
        [this](const int size) {
          const auto new_size = static_cast<size_t>(size);
          pool_.tt_->resize(new_size);
        });

    auto thread_count = option_callback(
        spin_option("Threads", default_thread_count, spin_range{1, 512}),
  ...
  void set_position(const std::string& line) {
    const std::regex startpos_with_moves(
        "position startpos moves((?: [a-h][1-8][a-h][1-8]+(?:q|r|b|n)?)+)");
    const std::regex fen_with_moves(
        "position fen (.*) moves((?: [a-h][1-8][a-h][1-8]+(?:q|r|b|n)?)+)");
  ...
  void info_string(const T& worker) {
    ...
    const search::score_type scaled_score =
        raw_score * raw_multiplier / raw_divisor;
    ...
    const size_t nps =
        std::chrono::milliseconds(std::chrono::seconds(1)).count() * nodes /
        (1 + elapsed_ms);
    if (is_searching()) {
      os << "info depth " << depth << " seldepth "
         << worker.internal.stack.sel_depth() << " score cp " << score
         << " nodes " << nodes << " nps " << nps << " time " << elapsed_ms
         << " pv " << worker.internal.stack.pv_string() << std::endl;
    }
  }
  ...
  void stop() {
    ...
    pool_.stop();
    os << "bestmove "
       << pool_.primary_worker().best_move().name(position.turn()) << std::endl;
  }
  ...
  void id_info() {
    std::lock_guard<std::mutex> os_lk(os_mutex_);
    os << "id name " << version::engine_name << " " << version::major << '.'
       << version::minor << '.' << version::patch << std::endl;
    os << "id author " << version::author_name << std::endl;
    os << options();
    if
      constexpr(search::constants::tuning) {
        os << (pool_.constants_->options());
      }
    os << "uciok" << std::endl;
  }
  ...
  void read(const std::string& line) {
    ...
    if (!is_searching() && line == "uci") {
      ...
      if
        constexpr(search::constants::tuning) {
          (pool_.constants_->options()).update(line);
        }
    }
  }

  uci()
      : pool_(&weights_, default_hash_size,
              [this](const auto& worker) { info_string(worker); },
              [this](const auto& worker) {
                if (manager_.should_stop(
                        search_info{worker.depth(), worker.is_stable()})) {
                  stop();
                }
              }) {
    nnue::embedded_weight_streamer<weight_type> embedded(
