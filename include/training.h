...
std::tuple<bool, search::wdl_type> terminality(
    const chess::position_history& hist, const state_type& state) {
  using return_type = std::tuple<bool, search::wdl_type>;

  if (hist.is_two_fold(state.hash())) {
    return return_type(true, draw);
  }
  if (state.generate_moves().size() == 0) {
    return return_type(true, state.is_check() ? loss : draw);
  }
...
struct train_interface {
  std::shared_ptr<search::constants> constants_ =
      std::make_shared<search::constants>(1);
  std::shared_ptr<chess::transposition_table> tt_ =
      std::make_shared<chess::transposition_table>(config::tt_mb_size);
  ...
  search::wdl_type get_wdl(const continuation_type& continuation) const {
    if (const auto[is_terminal, wdl] =
            terminality(continuation.history(), continuation.state());
        is_terminal) {
      return wdl;
    }
  ...
  std::optional<continuation_type> get_continuation(state_type state) {
    ...
    if (const auto terminal = terminality(hist, state);
        std::get<bool>(terminal)) {
      return continuation_type{hist, state};
    }

    std::unique_ptr<worker_type> worker = std::make_unique<worker_type>(
        &weights_, tt_, constants_, [&timer, &worker](auto&&...) {
          if (timer.elapsed() >= config::timeout) {
            worker->stop();
          }
          if (worker->depth() >= config::continuation_depth) {
            worker->stop();
          }
        });
    ...
    for (search::depth_type length{0}; length < config::continuation_max_length;
         ++length) {
      if (const auto terminal = terminality(hist, state);
          std::get<bool>(terminal)) {
        return continuation_type{hist, state};
      }
      if (last_move.is_quiet() && state.num_pieces() != man_0) {
        return continuation_type{hist, state};
      }
