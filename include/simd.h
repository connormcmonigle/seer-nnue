...
inline T dot_product(const T* a, const T* b){
  if constexpr(std::is_same_v<T, float>){
    ...
    for(size_t i(0); i < N; i += per_iteration, a += per_iteration, b += per_iteration){
      {
        const __m128 a_0 = _mm_load_ps(a + 0*per_unit); const __m128 b_0 = _mm_load_ps(b + 0*per_unit);
        sum_0 = _mm_add_ps(_mm_mul_ps(a_0, b_0), sum_0);
      }

      {
        const __m128 a_1 = _mm_load_ps(a + 1*per_unit); const __m128 b_1 = _mm_load_ps(b + 1*per_unit);
        sum_1 = _mm_add_ps(_mm_mul_ps(a_1, b_1), sum_1);
      }
      
      {
        const __m128 a_2 = _mm_load_ps(a + 2*per_unit); const __m128 b_2 = _mm_load_ps(b + 2*per_unit);
        sum_2 = _mm_add_ps(_mm_mul_ps(a_2, b_2), sum_2);
      }

      {
        const __m128 a_3 = _mm_load_ps(a + 3*per_unit); const __m128 b_3 = _mm_load_ps(b + 3*per_unit);
