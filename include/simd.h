  ...
  if
    constexpr(std::is_same_v<T, float>) {
      ...
      static_assert(alignment % sizeof(T) == 0,
                    "alignment must be divisible by sizeof(T)");
      ...
      static_assert(N % per_iteration == 0,
                    "N must be divisible by per_iteration");
      __m256 sum_0 = _mm256_setzero_ps();
      __m256 sum_1 = _mm256_setzero_ps();

      for (size_t i(0); i < N;
           i += per_iteration, a += per_iteration, b += per_iteration) {
        {
      ...
      // avoids extra move instruction by casting sum, adds lower 4 float
      // elements to upper 4 float elements
      const __m128 reduced_4 = _mm_add_ps(_mm256_castps256_ps128(reduced_8),
                                          _mm256_extractf128_ps(reduced_8, 1));
      // adds lower 2 float elements to the upper 2
      const __m128 reduced_2 =
          _mm_add_ps(reduced_4, _mm_movehl_ps(reduced_4, reduced_4));
      // adds 0th float element to 1st float element
      const __m128 reduced_1 =
          _mm_add_ss(reduced_2, _mm_shuffle_ps(reduced_2, reduced_2, 0x1));
      ...
      const __m128 reduced_4 =
          _mm_add_ps(_mm_add_ps(sum_0, sum_1), _mm_add_ps(sum_2, sum_3));
      // adds lower 2 float elements to the upper 2
      const __m128 reduced_2 =
          _mm_add_ps(reduced_4, _mm_movehl_ps(reduced_4, reduced_4));
      // adds 0th float element to 1st float element
