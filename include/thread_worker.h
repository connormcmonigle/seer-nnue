...
struct thread_worker {
  ...
  auto pv_search(
    ...
    // step 9. prob pruning
    const bool prob_prune = !is_pv && !ss.has_excluded() && !threatened.any() && maybe.has_value() &&
                            depth >= external.constants->prob_prune_depth() && maybe->best_move().is_capture() &&
                            maybe->bound() == bound_type::lower && maybe->score() > beta + external.constants->prob_prune_margin() &&
