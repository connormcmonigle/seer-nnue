...
struct thread_worker {
  ...
  auto pv_search(
    ...
    for (const auto& [idx, mv] : orderer) {
      ...
      const search::counter_type history_value = internal.hh.us(bd.turn()).compute_value(history::context{is_check, follow, counter}, mv);
      const search::see_type see_value = bd.see<search::see_type>(mv);
    ...
    if (loop.keep_going() && !ss.has_excluded()) {
      ...
      if (bound == bound_type::lower && best_move.is_quiet()) {
        internal.hh.us(bd.turn()).update(history::context{is_check, follow, counter}, best_move, quiets_tried, depth);
