  ...
  auto pv_search(
    ...
    for (const auto& [idx, mv] : orderer) {
      ...
      const search::score_type score = [&, this, idx = idx, mv = mv] {
        ...
        const bool is_pv_mv = is_pv && idx == 0;

        // search again at full depth if necessary
        if (!is_pv_mv && (!try_lmr || (zw_score > alpha && lmr_depth < next_depth))) { zw_score = zero_width(next_depth); }

        // search again with full window on pv nodes
        return (is_pv_mv || (alpha < zw_score && zw_score < beta)) ? full_width() : zw_score;
      }();
