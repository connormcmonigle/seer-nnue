...
struct external_state {
  ...
  external_state(const nnue::weights<typename T::weight_type>* weights_,
...
struct controlled_loop {
  ...
  void loop_(std::function<void()> f) {
    for (;;) {
      if (kill_.load(std::memory_order_relaxed)) {
        break;
      }
      std::unique_lock<std::mutex> control_lk(control_mutex_);
      cv_.wait(control_lk, [this] {
        return go_.load(std::memory_order_relaxed) ||
               kill_.load(std::memory_order_relaxed);
      });
      f();
  ...
  controlled_loop<is_active>& operator=(
      const controlled_loop<is_active>& other) = delete;
  controlled_loop<is_active>& operator=(controlled_loop<is_active>&& other) =
      delete;
...
struct thread_worker {
  ...
  template <bool is_pv>
  search::score_type q_search(const search::stack_view& ss,
                              const nnue::eval<T>& eval, const board& bd,
    ...
    if (list.size() == 0 && is_check) {
      return ss.effective_mate_score();
    }
    if (ss.is_two_fold(bd.hash())) {
    ...
    move_orderer orderer(move_orderer_data{move::null(), move::null(),
                                           move::null(), &bd, list,
                                           &internal.hh.us(bd.turn())});

    const std::optional<transposition_table_entry> maybe =
        external.tt->find(bd.hash());
    if (maybe.has_value()) {
    ...
    const search::score_type static_eval = [&] {
      ...
      if (!is_check) {
        internal.cache.insert(bd.hash(), val);
      }

      if (maybe.has_value()) {
        if (maybe->bound() == bound_type::upper && val > maybe->score()) {
          return maybe->score();
        }
        if (maybe->bound() == bound_type::lower && val < maybe->score()) {
          return maybe->score();
        }
      }
      return val;
    ...
    if (list.size() == 0 || static_eval >= beta) {
      return static_eval;
    }
    if (ss.reached_max_height()) {
      return static_eval;
    }
    ...
    for (const auto & [ idx, mv ] : orderer) {
      assert((mv != move::null()));
      if (!loop.keep_going() || best_score >= beta) {
        break;
      }
      ...
      const search::score_type score =
          -q_search<is_pv>(ss.next(), eval_, bd_, -beta, -alpha, elevation + 1);
      ...
      if (score > best_score) {
        ...
        if (score > alpha) {
          ...
          if
            constexpr(is_pv) { ss.prepend_to_pv(mv); }
        }
      }
    }
    ...
    if (loop.keep_going()) {
      const bound_type bound =
          best_score >= beta ? bound_type::lower : bound_type::upper;
      const transposition_table_entry entry(bd.hash(), bound, best_score,
                                            best_move, 0);
  ...
  auto pv_search(const search::stack_view& ss, const nnue::eval<T>& eval,
                 const board& bd, search::score_type alpha,
                 const search::score_type& beta, search::depth_type depth)
      -> pv_search_result_t<is_root> {
    auto make_result = [](const search::score_type& score, const move& mv) {
      if
        constexpr(is_root) { return pv_search_result_t<is_root>{score, mv}; }
      if
        constexpr(!is_root) { return score; }
    };
    ...
    if (loop.keep_going()) {
      external.on_update(*this);
    }

    // step 1. drop into qsearch if depth reaches zero
    if (depth <= 0) {
      return make_result(q_search<is_pv>(ss, eval, bd, alpha, beta, 0),
                         move::null());
    }
    ++internal.nodes;
    ...
    const bool is_check = bd.is_check();
    if (list.size() == 0 && is_check) {
      return make_result(ss.effective_mate_score(), move::null());
    }
    if (list.size() == 0) {
      return make_result(search::draw_score, move::null());
    }
    ...
    move_orderer orderer(move_orderer_data{killer, follow, counter, &bd, list,
                                           &internal.hh.us(bd.turn())});
    const std::optional<transposition_table_entry> maybe =
        external.tt->find(bd.hash());
    if (maybe.has_value()) {
    ...
    if (ss.reached_max_height()) {
      return make_result(static_eval, move::null());
    }
    ...
    const bool snm_prune =
        !is_root && !is_pv && !is_check &&
        depth <= external.constants->snmp_depth() &&
        static_eval >
            beta + external.constants->snmp_margin(improving, depth) &&
    ...
    const bool try_nmp = !is_root && !is_pv && !is_check &&
                         depth >= external.constants->nmp_depth() &&
                         static_eval > beta && ss.nmp_valid() &&
    ...
    if (try_nmp) {
      ...
      const search::score_type nmp_score =
          -pv_search<is_pv>(ss.next(), eval, bd.forward(move::null()), -beta,
                            -alpha, adjusted_depth);
      if (nmp_score > beta) {
    ...
    for (const auto & [ idx, mv ] : orderer) {
      ...
      const search::counter_type history_value =
          internal.hh.us(bd.turn()).compute_value(follow, counter, mv);

      const board bd_ = bd.forward(mv);

      const bool try_pruning = !is_root && !is_pv && !bd_.is_check() &&
                               !is_check && idx != 0 && mv.is_quiet() &&
                               best_score > ss.effective_mate_score();

      // step 9. pruning
      if (try_pruning) {
        const bool lm_prune =
            depth <= external.constants->lmp_depth() &&
            quiets_tried.size() >
                external.constants->lmp_count(improving, depth);
        ...
        const bool history_prune =
            ...
            history_value <=
                external.constants->history_prune_threshold(improving, depth);
      ...
      const search::depth_type extension = [&, mv = mv ] {
        const bool check_ext =
            bd.see<search::see_type>(mv) > 0 && bd_.is_check();
        ...
        const bool history_ext =
            !is_root && maybe.has_value() && mv == maybe->best_move() &&
            mv.is_quiet() &&
      ...
      const search::score_type score = [&, this, idx = idx, mv = mv ] {
        ...
        auto full_width = [&] {
          return -pv_search<is_pv>(ss.next(), eval_, bd_, -beta, -alpha,
                                   next_depth);
        };

        const bool try_lmr =
            !is_check && (mv.is_quiet() || bd.see<search::see_type>(mv) < 0) &&
            idx != 0 && (depth >= external.constants->reduce_depth());
        search::score_type zw_score{};

        // step 11. late move reductions
        if (try_lmr) {
          search::depth_type reduction =
              external.constants->reduction(depth, idx);

          // adjust reduction
          if (bd_.is_check()) {
            --reduction;
          }
          if (bd.is_passed_push(mv)) {
            --reduction;
          }
          if (!improving) {
            ++reduction;
          ...
          if (bd.see<search::see_type>(mv) < 0 && mv.is_quiet()) {
            ++reduction;
          }
          ...
          const search::depth_type lmr_depth =
              std::max(1, next_depth - reduction);
          zw_score = -pv_search<false>(ss.next(), eval_, bd_, -alpha - 1,
                                       -alpha, lmr_depth);
        }
      ...
      if (score < beta && mv.is_quiet()) {
        quiets_tried.add_(mv);
      }
    ...
    // step 12. update histories if appropriate and maybe insert a new
    // transposition_table_entry
    if (loop.keep_going()) {
      if (best_score >= beta) {
        const transposition_table_entry entry(bd.hash(), bound_type::lower,
                                              best_score, best_move, depth);
        external.tt->insert(entry);
        if (best_move.is_quiet()) {
          internal.hh.us(bd.turn())
              .update(follow, counter, best_move, quiets_tried, depth);
          ss.set_killer(best_move);
        }
      } else {
        const transposition_table_entry entry(bd.hash(), bound_type::upper,
                                              best_score, best_move, depth);
        external.tt->insert(entry);
      }
    }
  ...
  void iterative_deepening_loop_() {
    ...
    for (; loop.keep_going() &&
           internal.depth <= (external.constants->max_depth());
         ++internal.depth) {
      ...
      for (;;) {
        ...
        const search::depth_type adjusted_depth =
            std::max(1, internal.depth - failed_high_count);
        const auto[search_score, search_move] = pv_search<true, true>(
            search::stack_view::root(internal.stack), evaluator,
            internal.stack.root_pos(), alpha, beta, adjusted_depth);
  ...
  void go(const position_history& hist, const board& bd,
          const search::depth_type& start_depth) {
  ...
  thread_worker(
      const nnue::weights<T>* weights, std::shared_ptr<transposition_table> tt,
      std::shared_ptr<search::constants> constants,
      std::function<void(const thread_worker<T, is_active>&)> on_iter =
          [](auto&&...) {},
      std::function<void(const thread_worker<T, is_active>&)> on_update =
          [](auto&&...) {})
...
struct worker_pool {
  ...
  void go(const position_history& hist, const board& bd) {
    ...
    for (size_t i(0); i < pool_.size(); ++i) {
      const search::depth_type start_depth =
          1 + static_cast<search::depth_type>(i % 2);
  ...
  size_t nodes() const {
    return std::accumulate(pool_.begin(), pool_.end(), static_cast<size_t>(0),
                           [](const size_t& count, const auto& worker) {
  ...
  worker_pool(const nnue::weights<T>* weights, size_t hash_table_size,
              std::function<void(const thread_worker<T, true>&)> on_iter =
                  [](auto&&...) {},
              std::function<void(const thread_worker<T, true>&)> on_update =
                  [](auto&&...) {})
      : weights_{weights} {
    tt_ = std::make_shared<transposition_table>(hash_table_size);
    constants_ = std::make_shared<search::constants>();
    pool_.push_back(std::make_unique<thread_worker<T>>(weights, tt_, constants_,
                                                       on_iter, on_update));
  }
};
}
