...
struct fixed_search_constants {
  ...
  [[nodiscard]] constexpr depth_type reduce_depth() const noexcept { return 1; }
  ...
  [[nodiscard]] constexpr depth_type nmp_reduction(const depth_type& depth, const score_type& beta, const score_type& value) const noexcept {
    return 3 + depth / 4 + std::min(3, (value - beta) / 242);
  }

  [[nodiscard]] constexpr see_type nmp_see_threshold() const noexcept { return 209; }
  ...
  [[nodiscard]] constexpr score_type singular_double_extension_margin() const noexcept { return 165; }

  [[nodiscard]] constexpr score_type futility_margin(const depth_type& depth) const noexcept {
    constexpr score_type m = 1541;
  ...
  [[nodiscard]] constexpr see_type quiet_see_prune_threshold(const depth_type& depth) const noexcept { return -59 * static_cast<see_type>(depth); }
  [[nodiscard]] constexpr see_type noisy_see_prune_threshold(const depth_type& depth) const noexcept { return -124 * static_cast<see_type>(depth); }

  [[nodiscard]] constexpr counter_type history_prune_threshold(const depth_type& depth) const noexcept {
    return -1357 * static_cast<counter_type>(depth * depth);
  }
  ...
  [[nodiscard]] constexpr depth_type history_reduction(const counter_type& history_value) const noexcept {
    ...
    const depth_type raw = -static_cast<depth_type>(history_value / 5615);
    return std::clamp(raw, -limit, limit);
  }

  [[nodiscard]] constexpr score_type delta_margin() const noexcept {
    constexpr score_type margin = 514;
    return margin;
  }

  [[nodiscard]] constexpr see_type good_capture_prune_see_margin() const noexcept { return 309; }
  ...
  [[maybe_unused]] fixed_search_constants& update_(const std::size_t& thread_count) noexcept {
    ...
    for (depth_type depth{1}; depth < lmr_tbl_dim; ++depth) {
      for (depth_type played{1}; played < lmr_tbl_dim; ++played) {
        lmr_tbl[depth * lmr_tbl_dim + played] = static_cast<depth_type>(0.5354240371831789 + std::log(depth) * std::log(played) / 2.492572118395889);
      }
    }
    return *this;
  }
