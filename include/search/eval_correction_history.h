...
struct eval_correction_history {
  ...
  [[nodiscard]] static constexpr std::tuple<score_type, score_type> filter_coefficients(const depth_type& depth) noexcept {
    const score_type alpha = 256 * depth;
    const score_type c_alpha = 65536;
    return std::make_tuple(alpha, c_alpha);
  }
  ...
  constexpr void update(const zobrist::quarter_hash_type& feature_hash, const score_type& error, const depth_type& depth) noexcept {
    ...
    const auto [filter_alpha, filter_c_alpha] = filter_coefficients(depth);
    const score_type filter_divisor = filter_alpha + filter_c_alpha;
