...
std::ostream& operator<<(std::ostream& os, const bench_info& info) {
...
bench_info get_bench_info(const nnue::weights<T>& weights) {
  ...
  std::shared_ptr<chess::transposition_table> tt =
      std::make_shared<chess::transposition_table>(bench_config::tt_mb_size);

  std::unique_ptr<worker_type> worker = std::make_unique<worker_type>(
      &weights, tt, constants, [&](const auto& w) {
        if (w.depth() >= bench_config::bench_depth) {
          worker->stop();
        }
      });
  ...
  for (const auto& fen : bench_config::fens) {
    worker->go(chess::position_history{},
               chess::board::parse_fen(std::string(fen)),
               bench_config::init_depth);
    worker->iterative_deepening_loop_();
    total_nodes += worker->nodes();
  }

  const size_t nodes_per_second =
      total_nodes * std::chrono::milliseconds(std::chrono::seconds(1)).count() /
      timer.elapsed().count();
