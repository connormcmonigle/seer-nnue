...
constexpr std::uint32_t make_positive(const std::int32_t& x) {
  constexpr std::uint32_t upper =
      static_cast<std::uint32_t>(1) + std::numeric_limits<std::int32_t>::max();
  return upper + x;
}
...
struct move_orderer_data {
  ...
  move_orderer_data(const move& killer_, const move& follow_,
                    const move& counter_, const board* bd_,
                    const move_list& list_, const history_heuristic* hh_)
      : killer{killer_},
        follow{follow_},
        counter{counter_},
        bd{bd_},
...
struct move_orderer_entry {
  ...
  move_orderer_entry(const move& mv_, bool is_first, bool is_noisy,
                     bool is_killer, std::int32_t value)
      : mv{mv_} {
...
struct move_orderer_iterator {
  ...
  move_orderer_iterator& operator++() {
    ++idx_;
    if (idx_ < static_cast<int>(move_count_)) {
      update_list_();
    }
    return *this;
  }
  ...
  move_orderer_iterator(const move_orderer_data& data, const int& idx)
      : move_count_{data.list.size()}, idx_{idx} {
    std::transform(
        data.list.begin(), data.list.end(), entries_.begin(),
        [&data](const move& mv) {
          const bool quiet = mv.is_quiet();
          const std::int32_t value =
              quiet ? data.hh->compute_value(data.follow, data.counter, mv)
                    : data.bd->see<std::int32_t>(mv);
          return move_orderer_entry(mv, mv == data.first, !quiet && value >= 0,
                                    quiet && mv == data.killer, value);
        });
    update_list_();
  }
...
struct move_orderer {
  ...
  move_orderer_iterator end() {
    return move_orderer_iterator(data_.list.size());
  }
