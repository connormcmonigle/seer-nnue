...
inline constexpr std::array<piece_type, 4> promotion_types = {
    piece_type::knight, piece_type::bishop, piece_type::rook,
    piece_type::queen};
...
struct move {
  ...
  constexpr square from() const {
    return square::from_index(get_field_<from_>());
  }
  ...
  constexpr square enpassant_sq() const {
    return square::from_index(get_field_<enpassant_sq_>());
  }
  ...
  constexpr bool is_castle_oo() const {
    return piece() == piece_type::king && from() == castle_info<c>.start_king &&
  ...
  constexpr bool is_quiet() const {
    return !is_capture() &&
           !(is_promotion() && piece_type::queen == promotion());
  }

  template <color c>
  std::string name() const {
    if (is_castle_oo<c>()) {
      return castle_info<c>.start_king.name() +
             castle_info<c>.after_oo_king.name();
    } else if (is_castle_ooo<c>()) {
      return castle_info<c>.start_king.name() +
             castle_info<c>.after_ooo_king.name();
    }
  ...
  constexpr move(square from, square to, piece_type piece,
...
std::ostream& operator<<(std::ostream& ostr, const move& mv) {
  ostr << "move(from=" << mv.from().name() << ", to=" << mv.to().name()
       << ", piece=" << piece_name(mv.piece())
       << ", is_capture=" << mv.is_capture()
       << ", capture=" << piece_name(mv.captured())
       << ", is_enpassant=" << mv.is_enpassant()
       << ", enpassant_sq=" << mv.enpassant_sq().name()
