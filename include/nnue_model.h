...
struct feature_transformer {
  const big_affine<T, feature::half_ka::numel, base_dim>* weights_;
  ...
  feature_transformer(const big_affine<T, feature::half_ka::numel, base_dim>* src) : weights_{src} { clear(); }
};
