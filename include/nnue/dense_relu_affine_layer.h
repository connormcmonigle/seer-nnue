...
struct dense_relu_affine_layer {
  ...
  [[nodiscard]] inline aligned_vector<dot_type<T>, dim1> forward_relu(const aligned_vector<T, dim0>& x) const noexcept {
    auto result = aligned_vector<dot_type<T>, dim1>::from(b);
    simd::relu_matrix_vector_product<dim0, dim1>(W, x.data, result.data);
    return result;
  }

  [[nodiscard]] inline aligned_vector<dot_type<T>, dim1> forward_relu(const aligned_slice<T, dim0>& x) const noexcept {
    auto result = aligned_vector<dot_type<T>, dim1>::from(b);
    simd::relu_matrix_vector_product<dim0, dim1>(W, x.data, result.data);
    return result;
  }

  [[nodiscard]] inline aligned_vector<dot_type<T>, dim1> forward_crelu255(const aligned_vector<T, dim0>& x) const noexcept {
    auto result = aligned_vector<dot_type<T>, dim1>::from(b);
    simd::crelu255_matrix_vector_product<dim0, dim1>(W, x.data, result.data);
    return result;
  }

  [[nodiscard]] inline aligned_vector<dot_type<T>, dim1> forward_crelu255(const aligned_slice<T, dim0>& x) const noexcept {
    auto result = aligned_vector<dot_type<T>, dim1>::from(b);
    simd::crelu255_matrix_vector_product<dim0, dim1>(W, x.data, result.data);
    return result;
  }
