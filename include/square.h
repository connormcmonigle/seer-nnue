...
struct square {
  ...
  std::string name() const {
    constexpr std::array<char, 8> n_of_file = {'h', 'g', 'f', 'e',
                                               'd', 'c', 'b', 'a'};
    constexpr std::array<char, 8> n_of_rank = {'1', '2', '3', '4',
                                               '5', '6', '7', '8'};
  ...
  constexpr static square from_index(const I index) {
    static_assert(std::is_integral_v<I>,
                  "square index must be of integral type");
    return square(static_cast<std::uint64_t>(1)
...
struct tbl_square {
  ...
  constexpr std::uint64_t bit_board() const {
    return static_cast<std::uint64_t>(1) << index();
  }
  ...
  constexpr tbl_square add(delta d) const {
    return tbl_square{file + d.x, rank + d.y};
  }
  ...
  static tbl_square from_name(const std::string& name) {
    return tbl_square{7 - static_cast<int>(name[0] - 'a'),
                      static_cast<int>(name[1] - '1')};
  }
};

template <typename T>
inline constexpr bool is_square_v =
    std::is_same_v<T, square> || std::is_same_v<T, tbl_square>;
...
std::ostream&
operator<<(std::ostream& ostr, const square_set& ss) {
