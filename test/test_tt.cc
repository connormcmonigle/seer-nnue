...
int main(){
  ...
  std::cout << "mv: " << mv << std::endl;
  {
    ...
    const auto entry_0 = chess::tt_entry(hash_0, chess::bound_type::lower, 102, mv, 10);
    const auto entry_1 = chess::tt_entry(hash_1, chess::bound_type::upper, 308, mv, 5);
...

  {
    ...
    const auto entry_0 = chess::tt_entry(hash, chess::bound_type::lower, 102, mv, 5);
    const auto entry_1 = chess::tt_entry(hash, chess::bound_type::upper, 960, mv, 6);
...

  {
    ...
    chess::table tt(tbl_size);
    const auto hash = zobrist::random_bit_string();
    const auto entry_0 = chess::tt_entry(hash, chess::bound_type::upper, 409, mv, 7);
    const auto entry_1 = chess::tt_entry(hash, chess::bound_type::lower, 960, mv, 8);
