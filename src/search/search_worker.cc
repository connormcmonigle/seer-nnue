...
pv_search_result_t<is_root> search_worker::pv_search(
  ...
  const bool spc_prune = !is_pv && !ss.has_excluded() && maybe.has_value() && depth >= 5 &&
                         maybe->best_move().is_capture() && maybe->bound() != bound_type::upper && maybe->score() >= beta + 768 &&
                         maybe->depth() + 2 >= depth;

  if (spc_prune) { return make_result(maybe->score(), chess::move::null()); }
