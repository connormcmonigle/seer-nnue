...
pv_search_result_t<is_root> search_worker::pv_search(
  ...
  bool early_lm_break_attempt{false};
  ...
  for (const auto& [idx, mv] : orderer) {
    ...
    if (try_pruning) {
      ...
      const bool early_lm_prune = !bd_.is_check() && depth <= external.constants->lmp_depth() && idx + 1 > external.constants->lmp_count(improving, depth);

      if (early_lm_prune && internal.lmphist.us(bd.turn()).should_early_lm_prune(depth)) {
        break;
      } else if (early_lm_prune) {
        early_lm_break_attempt = true;
      }
    ...
    if (early_lm_break_attempt) { internal.lmphist.us(bd.turn()).update(depth, score <= alpha); }
