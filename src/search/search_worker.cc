...
pv_search_result_t<is_root> search_worker::pv_search(
  ...
  if (try_razor) {
    ...
    const score_type reverse_razor_beta = beta + external.constants->snmp_margin(improving, threatened.any(), depth);
    if (razor_score >= reverse_razor_beta) {
      const score_type adjusted_score = (beta + razor_score) / 2;
      return make_result(adjusted_score, chess::move::null());
    }
  }
