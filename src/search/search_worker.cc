...
pv_search_result_t<is_root> search_worker::pv_search(
  ...
  const bool try_probcut =
      !is_pv && !ss.has_excluded() && depth >= external.constants->probcut_depth() && !(maybe.has_value() && maybe->best_move().is_quiet()) &&
      !(maybe.has_value() && maybe->depth() >= probcut_depth && maybe->bound() == bound_type::upper && maybe->score() < probcut_beta);
