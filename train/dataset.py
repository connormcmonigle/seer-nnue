...
class Data(torch.utils.data.Dataset):
  def __init__(self, config):
    ...
    mm_pov, mm_white, mm_black, mm_outcome, mm_score = util.get_memmap_handlers(mode='r', config=config)
    self.mm_pov = mm_pov
    self.mm_white = mm_white
    self.mm_black = mm_black
    self.mm_outcome = mm_outcome
  ...
  def sample(self):
    ...
    vals = [np.array([self.mm_pov[idx]]), np.copy(self.mm_white[idx]), np.copy(self.mm_black[idx]), np.array([self.mm_outcome[idx]]), np.array([self.mm_score[idx]])]
  ...
  def sample_batch(self):
    ...
    return torch.stack(pov_ls, dim=0).to(self.device).float(),\
      ...
      torch.stack(outcome_ls, dim=0).to(self.device).float(),\
